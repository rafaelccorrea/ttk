...
| 3c.7 | Chat manda objeção sem "?" ("tá caro", "é golpe?", "não confio") | `isQuestion=true` (palavras de objeção entraram em `PALAVRAS_INTERROGATIVAS`) e a resposta usa a FAQ tipo `objecao` | ✅ 25/08 run 12feee5f: "tá caro 😕" → enviar 0,97 |
| 3c.8 | Resposta copiada da FAQ sem `productIds` (pergunta conceitual: "é gravada?", "é seguro?") | Sai como **pronta**: `ancoradaNaFaq` (≥60% das palavras numa resposta da FAQ) conta como fonte. Resposta com URL escala sempre (regra de link) — não pôr URL na FAQ | ✅ 25/08 run 02946390: 16/16 prontas, 0 escalações (antes 8/17) |
| 3c.5 | `npm run live:processar-gravacao <email> <mp4> --contexto "..."` | Base criada na conta com o mesmo pipeline do upload; cobra transcrição por bloco + extração; imprime produtos, FAQ e transcrição | ⚠️ 25/08 — live de apresentação (22m42s): transcreveu (18 cr), extração voltou VAZIA porque o prompt procura produto físico; base montada com `npm run live:base-apresentacao <sessionId>` (13 produtos = planos, pacotes e módulos, com variações; 155 FAQ). `live:extrair-transcricao` retoma só a extração de uma sessão já transcrita |
...
| 6.14 | Encerrar a LIVE pelo TikTok (tela "LIVE encerrada") com o copiloto conectado | Em segundos o painel sai de "Lendo o chat" para **encerrada** com motivo "A live foi encerrada no TikTok."; run no backend `encerrada` / `endReason=live_encerrada` (não `erro`); fila de envio parada, nada mais postado. Queda de rede continua só avisando e reconectando (não encerra) | ⬜ |
