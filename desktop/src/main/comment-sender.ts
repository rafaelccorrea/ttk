...
import { montarEsqueleto } from './esqueleto-html';
 ...
 *
 * A regra em si mora em `esqueleto-html.ts` e é SERIALIZADA para dentro da
 * página — não reescrita aqui. A extração foi feita para que ela pudesse ser
 * testada (uma string injetada não se chama num teste), e o `toString()` é o que
 * impede o efeito colateral clássico dessa manobra: uma segunda cópia da lógica,
 * neste arquivo, envelhecendo em silêncio enquanto a testada é corrigida. O que
 * roda no tiktok.com é, literalmente, a função que o teste exercita.
 */
function scriptDeEsqueleto(): string {
  return `(() => {
    if (!document.body) return '';
    const montar = ${montarEsqueleto.toString()};
    return montar(document.body, window.innerHeight * (2 / 3), 8000);
  })()`;
}
