/**
 * CÓPIA de `frontend/src/theme/theme.ts`.
 *
 * É cópia, e não um pacote compartilhado, porque um workspace só para o tema
 * custaria mais manutenção do que o arquivo inteiro vale — mas isso tem um
 * preço: mudança de marca no frontend precisa ser trazida para cá na mão,
 * senão o desktop e o site divergem de cor no mesmo produto. As versões de
 * `@mui/material` e `@emotion/*` do desktop são casadas com as do frontend
 * exatamente para que esta cópia continue compilando sem adaptação.
 */
